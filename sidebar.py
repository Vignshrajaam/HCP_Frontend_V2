...
def load_hcp_data(file_bytes, filename):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if node_archives:
            first = node_archives[0]
            node_dir = os.path.join(tmpdir, Path(first).stem)
            os.makedirs(node_dir, exist_ok=True)
            with tarfile.open(first, "r:*") as t:
                t.extractall(node_dir)
        svc = HCPDataService(tmpdir)
...
def load_access_logs(file_bytes, filename):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(tmp_zip, "wb") as f:
            f.write(file_bytes)
        extract_all_archives_recursive(tmp_zip, tmpdir)
        svc = HCPDataService(tmpdir)
        data = svc.gather_all(include_access_logs=True)
    return data.get("access_logs")
       ...
       # TITLE + INFO ICON
        st.markdown(
            ...
            <div style="text-align:center; padding:25px 0 5px;">
                ...
                <span style="font-size:12px;color:#888;">V2.0</span>
                &nbsp;
                <span class="info-wrap">
                    <span style="cursor:pointer;font-size:14px;color:#aaa;">&#x24D8;</span>
                    <div class="info-tip">
                        <b style="display:block;margin-bottom:6px;font-size:12px;"><center>Build Info</center></b>
                        <div class="tip-row">
                            <span class="tip-label"><b>Project</b></span>
                            HCP SmartAnalytics
                        </div>
                        <div class="tip-row">
                            <span class="tip-label"><b>Version</b></span>
                            2.0
                        </div>
                        <div class="tip-row">
                            <span class="tip-label"><b>Frontend Build</b></span>
                            20260604 
                        </div>
                        <div class="tip-row">
                            <span class="tip-label"><b>Backend Build</b></span>
                            20260604
                        </div>
                    </div>
                </span>
        ...
        st.divider()
        st.page_link("pages/1_Cluster_summary.py",   label="1 · Cluster summary")
        st.page_link("pages/2_Config_insights.py",   label="2 · Config insights")
        st.page_link("pages/3_Tenants_namespaces.py",label="3 · Tenants & namespaces")
        st.page_link("pages/4_Namespace_insights.py",label="4 · Namespace insights")
        st.page_link("pages/6_Query_engine.py",      label="5 · Query engine")
        st.page_link("pages/7_Access_log_analysis.py",label="6 · Access log analysis")
        st.divider()
        ...
        if uploaded:
            try:
                ...
                # Reset access logs
                st.session_state.pop("access_logs", None)
                st.session_state["access_logs_loaded"] = False

                config = data.get("config", {})
                cfgs = config.get("cluster_cfg", [])
                if cfgs:
                    s = cfgs[0].get("parsed", {}).get("settings", {})
                    cname = s.get("clname", Path(uploaded.name).stem)
                    ver = s.get("software_version", "?")
                else:
                    cname = Path(uploaded.name).stem
                    ver = "?"
